...
from utils.trainer import FewShotTrainer, SchemaFewShotTrainer, LabelNumFewShotTrainer, LabelNumSchemaFewShotTrainer, prepare_optimizer
from utils.tester import FewShotTester, SchemaFewShotTester, LabelNumFewShotTester, LabelNumSchemaFewShotTester, eval_check_points
...
def main():
    ...
    if opt.use_schema:
        if opt.label_num_schema:
            trainer_class = LabelNumSchemaFewShotTrainer
            tester_class = LabelNumSchemaFewShotTester
        else:
            trainer_class = SchemaFewShotTrainer
            tester_class = SchemaFewShotTester
    else:
        if opt.label_num_schema:
            trainer_class = LabelNumFewShotTrainer
            tester_class = LabelNumFewShotTester
        else:
    ...
    if opt.do_train:
        ...
        upper_structures = [
            'backoff', 'scale_rate', 'f_theta', 'phi', 'start_reps', 'end_reps', 'biaffine', 'threshold',
            'bandwidth', 'map_linear']
