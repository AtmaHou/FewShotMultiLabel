...
class MiniIncludeGenerator(DataGeneratorBase):
    ...
    def get_label_bucket(self, domain_data: dict) -> Tuple[Dict[str, List[int]], List[List[str]]]:
        ...
        elif self.opt.task in ['mlc', 'sc']:  # use sentence labels
            label_field = 'labels'
        else:
    ...
    def sample_support_set(self, data_part, label_set, label_bucket, d_id2label):
        ...
        # ''' Step1.1 count the label num'''
        # selected_label_num_dict = {}
        # for d_id in selected_data_ids:
        #     for label in d_id2label[d_id]:
        #         if label in selected_label_num_dict:
        #             selected_label_num_dict[label] += 1
        #         else:
        #             selected_label_num_dict[label] = 1
        # print('selected_label_num_dict: {}'.format(selected_label_num_dict))
        # print('shot_counts: {}'.format(shot_counts))
        ...
        for d_id in selected_data_ids:
            ...
            for label in to_be_removed_labels:
                if label in shot_counts and shot_counts[label] - 1 < support_shot_num:
                    ...
                    break
            if can_remove:
                # print('sample_id: {}'.format(d_id))
                # print('to_be_removed_labels: {}'.format(to_be_removed_labels))
                if random.randint(1, 100) < self.opt.remove_rate:  # Not to remove all removable data to give chances to extreme cases.
                    ...
                    for label in to_be_removed_labels:
                        if label in shot_counts:
                            shot_counts[label] -= 1
                # print('in can remove: shot_counts: {}'.format(shot_counts))
        ...
        # if (len(selected_data['labels']) + len(remained_data['labels']) != len(data_part['labels'])) or len(
        #         selected_data) == 0 or len(remained_data) == 0:
        #     raise RuntimeError(
        #         "ERROR: Failed to collect remained or few shot data: few_shot {} remained {} total{}".format(
        #             len(selected_data['labels']), len(remained_data['labels']), len(data_part['labels'])))

        if self.opt.check:
        #     # check removal
        #     print('Num of selected items: BEFORE/AFTER REMOVAL {}/{}'. format(num_before, num_after))
    ...
    def update_label_bucket(self, sampled_id, tmp_label_bucket, d_id2label):
        ...
        for label in labels:
            ...
            # try:
            # except ValueError:
            #     print('del sample_id: {}'.format(sampled_id))
            #     print('tmp_label_bucket: {}'.format(tmp_label_bucket[label]))
            #     raise ValueError('the sample id not in tmp_label_bucket')
