...
class TourSGDataLoader(SLUDataLoader):
    ...
    def del_dis_fluency(self, tag_utterance):
        ...
        # tag_utterance_lst = [word for word in tag_utterance_lst if not word.startswith('%')]
        # new_tag_utterance = ' '.join(tag_utterance_lst)
        new_tag_utterance = new_tag_utterance.strip()
        return new_tag_utterance

    def get_bio(self, tag_utterance):
        # print('DEBUG: tag_utterance: {}'.format(tag_utterance))
        all_match = [m.group(0) for m in re.finditer(self.tag_ptn, tag_utterance)]
        target_dict = {}
        if all_match:
            for match in all_match:
                # print('DEBUG: match: {}'.format(match))
                ...
                if cat_type == 'MAIN':
                    ...
                    target_tag = cat_type
                # print('DEBUG: target_tag: {}'.format(target_tag))
        ...
        for _, tag_lst in target_dict.items():
            for item in tag_lst:
                ...
                tags[idx] = item[1]
        # print('DEBUG: tag_utterance_lst: {}'.format(tag_utterance_lst))
    ...
    def split_punc(self, utterance):
        # if utterance[-1] in self.punctuation:
...
class SMPDataLoader(RawDataLoaderBase):
    ...
    def unpack_train_data(self, data_path: str, remove_set: set = None):
        ...
        for item in json_data:
            ...
            # if remove_set is not None and item['text'] not in remove_set or remove_set is None:
            seq_in, seq_out, label = self.handle_one_utterance(item)
        ...
        for slot_key, slot_value in slots.items():
            ...
            for s_val in slot_value:
                ...
                if s_val in text:
                    ...
                    # raise ValueError('the slot is not in utterance text')
                    print('text: {}'.format(text))
                    print('  slot_key: {} - slot_value: {}'.format(slot_key, s_val))
                    # some slot value cannot be transferred by `seq_outs`
...
if __name__ == '__main__':
    ...
    # st_path = 'D:/Data/Project/Data/StanfordLabeled'
    # atis_path = 'D:/Data/Project/Data/atis'
    # toursg_path = '/Users/lyk/Work/Dialogue/data/DSTC4-TourSg/dstc4_traindev/data'
    # toursg_path = '/users7/xmshi/data/dstc4/data/'
    # smp_path = '/Users/lyk/Work/Dialogue/FewShot/SMP正式数据集/'
    smp_path = '/Users/lyk/Work/Dialogue/FewShot/SMP/'

    # st_loader = StanfordDataLoader(opt)
    # atis_loader = SLUDataLoader(opt)
    # toursg_loader = TourSGDataLoader(opt)
    ...
    print("support:  smp domain: {}".format(support_data.keys()))

    all_num = 0
    for key, value in dev_data.items():
        all_num += len(value['seq_ins'])
    print('all_num: {}'.format(all_num))

    # print("support: {}".format(support_data))
    # print("dev_data: {}".format(dev_data))

    # domains = support_data.keys()
    # dev_features = {}
    # for domain in domains:
    #     dev_features[domain] = [{'support': support_data[domain], 'query': dev_data[domain]}]
    #
    # with open('dev.json', 'w') as fw:
    #     json.dump(dev_features, fw, ensure_ascii=False)


    # a = st_loader.load_data(st_path)
    # print(a)
    # a = atis_loader.load_data(atis_path)
    # print(a)
    # data = toursg_loader.load_data(toursg_path, is_test=False)
    # domains = data.keys()
    # print('domains: {}'.format(domains))

    # '''multi-label utterance rate'''
    # multi_label_num = 0
    # all_num = 0
    # for domain in domains:
    #     domain_labels = data[domain]['labels']
    #     all_num += len(domain_labels)
    #     for labels in domain_labels:
    #         if len(labels) > 1:
    #             multi_label_num += 1
    # print('multi label rate {}/{} = {}'.format(multi_label_num, all_num, multi_label_num / all_num))
    #
    # '''label rate'''
    # label_dict = {}
    # for domain in domains:
    ...
    #             else:
    #                 label_dict[label] += 1
    # label_sum = sum([v for _, v in label_dict.items()])
    # label_lst = sorted(label_dict.items(), key=lambda x: -x[1])
    # label_rate_lst = [(v[0], round(v[1] / label_sum, 2)) for v in label_lst]
    # print('{} - label_dict: {} - {}'.format(opt.label_type, len(label_dict), label_lst))
    # print('\t label rate list: {}'.format(label_rate_lst))
    # # with open('tour_sg_raw_data', 'w') as fw:
    # #     json.dump(data, fw)
    #
    # '''little label multi-label rate'''
    # little_label_lst = [key[0] for key in label_lst[::-1][:3]]
    # little_label_rate_dict = {}
    # for domain in domains:
    #     domain_labels = data[domain]['labels']
    #     for labels in domain_labels:
    #         for l_label in little_label_lst:
    #             if l_label in labels:
    #                 if l_label not in little_label_rate_dict:
    #                     little_label_rate_dict[l_label] = [0, 0]
    #                 if len(labels) > 1:
    #                     little_label_rate_dict[l_label][0] += 1
    #                 little_label_rate_dict[l_label][1] += 1
    #
    # for l_label in little_label_lst:
    #     print('{} rate - {}/{} = {}'.format(l_label,
    #                                         little_label_rate_dict[l_label][0],
    #                                         little_label_rate_dict[l_label][1],
    #                                         round(little_label_rate_dict[l_label][0] / little_label_rate_dict[l_label][1], 2)))
    #
    # # tag_ptn = re.compile(r'<(\w+|\s|-|=|\\|\")+>\s*\w+(\s\w+)*<\/\w+>')
    # # left_tag_ptn = re.compile(r'<(\w+|\s|-|=|\\|\")+>')
    # # ut_str = "%uh and then maybe you can have a late <FOOD CAT=\"MAIN\">dinner</FOOD>."
    # # res = re.search(tag_ptn, ut_str)
    # # print('res: {}'.format(res))
    # # if res:
    # #     ut_str = ut_str.replace(res.group(0), '')
    # #     print('ut_str: {}'.format(ut_str))
    # # res2 = re.search(left_tag_ptn, ut_str)
    # # print('res2: {}'.format(res2))
    # # if res2:
    # #     ut_str = ut_str.replace(res2.group(0), '')
    # #     print('ut_str: {}'.format(ut_str))
    # # all_res = re.findall(tag_ptn, ut_str)
    # # print('all_res: {}'.format(all_res))
    # # iter_res = [m.group(0) for m in re.finditer(tag_ptn, ut_str)]
    # # print('iter_res: {}'.format(iter_res))
