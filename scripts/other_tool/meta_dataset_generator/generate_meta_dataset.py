...
# DEFAULT_INPUT_PATH = '/users7/xmshi/data/dstc4/data/'
# DEFAULT_OUTPUT_DIR = './MetaData/'
# DEFAULT_CONFIG_PATH = './other_tool/config/config{}.json'.format(DEFAULT_ID)
# DEFAULT_INPUT_PATH = '/Users/lyk/Work/Dialogue/data/DSTC4-TourSg/dstc4_traindev/data'
# DEFAULT_OUTPUT_DIR = './MetaData/'
# DEFAULT_CONFIG_PATH = '/Users/lyk/Work/Dialogue/data/DSTC4-TourSg/dstc4_traindev/scripts/config/config{}.json'.format(DEFAULT_ID)
...
def dump_data(opt, data):
    ...
    # )
    dump_dir_name = "{}.{}.spt_s_{}.q_s_{}.ep_{}{}".format(
        opt.dataset, opt.eval_config_id, opt.support_shots, opt.query_shot, opt.episode_num,
        '.fix_dev_spt' if opt.use_fix_support else ''
    )
    ...
    if 'fs' == opt.style:
        ...
        if opt.dataset == 'smp':
            ...
            if opt.use_fix_support:
                fix_spt_dev_meta_data = {}
                for domain in dev_meta_data.keys():
                    fix_spt_dev_meta_data[domain] = []
                    for m_item in dev_meta_data[domain]:
                        new_m_item = {'support': raw_data['support'][domain], 'query': m_item['query']}
                        fix_spt_dev_meta_data[domain].append(new_m_item)
                dev_meta_data = fix_spt_dev_meta_data
