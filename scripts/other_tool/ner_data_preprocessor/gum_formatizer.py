...
if __name__ == '__main__':
    train_lines = read_lines('./data/unformatted_data/gum/train.txt')
    valid_lines = []
    test_lines = read_lines('./data/unformatted_data/gum/test.txt')

    train_data = parse_lines(train_lines)
    valid_data = parse_lines(valid_lines)
    test_data = parse_lines(test_lines)

    rewrite_data(train_data, 'gum', 'train')
    rewrite_data(valid_data, 'gum', 'valid')
    rewrite_data(test_data, 'gum', 'test')
