...
from models.modules.text_classifier import MultiLabelTextClassifier, MetaStatsMultiLabelTextClassifier, \
    KRNMetaStatsMultiLabelTextClassifier
...
class FewShotTextClassifier(torch.nn.Module):
    ...
    def forward(
        ...
        # duplicate support reps to enable multi-label. (batch_size, support_size, multi-label_num, emb_len)
        if self.opt.task == 'mlc':
            support_reps = self.dup_support_reps(support_reps, support_target)

        # calculate emission: shape(batch_size, 1, no_pad_num_tag)
        test_output_mask = torch.ones(test_output_mask.shape[0], 1).to(test_output_mask.device)  # for sc & mlc, each test has only 1 output
        ...
        self.decoder: MultiLabelTextClassifier

        if type(self.decoder) == MetaStatsMultiLabelTextClassifier:  # update statistic for MS decoder
            self.decoder: MetaStatsMultiLabelTextClassifier
            self.decoder.update_statistics(support_target)
        elif type(self.decoder) == KRNMetaStatsMultiLabelTextClassifier:
            self.decoder: KRNMetaStatsMultiLabelTextClassifier
            self.decoder.update_statistics(support_target, support_sentence_feature, test_sentence_feature,
                                           support_sentence_target, test_sentence_target)

        if self.training:
            loss = self.decoder.forward(logits=logits, mask=test_output_mask, tags=test_target)
        else:
            if type(self.decoder) == KRNMetaStatsMultiLabelTextClassifier:
                prediction = self.decoder.decode(logits=logits, test_label_num=test_sentence_target)
            else:
    ...
    def dup_support_reps(self, support_reps, support_target):
        """
        For multi-label example, duplicate reps for m times. m is the number of labels it has.
        All support reps are expanded to max multi-label size,
        over expanded reps are eliminate by 0 support_target in emission scorer.
        :param support_reps: (batch_size, support_size, 1, emb_len)
        :param support_target: (batch_size, support_size, multi-label_num, num_tags)
        :return:
        """
        # todo: check pad influence
        max_label_num = support_target.shape[-2]
        expand_shape = list(support_reps.shape)
        expand_shape[-2] = max_label_num  # shape (batch_size, support_size, multi-label_num, emb_len)
        return support_reps.expand(expand_shape)
...
class SchemaFewShotTextClassifier(FewShotTextClassifier):
    ...
    def forward(
        ...
        if self.opt.task == 'mlc':
            ...
            raise TypeError('the task is not defined')
