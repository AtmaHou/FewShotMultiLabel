...
class FewShotFeature(object):
    ...
    def __init__(
            ...
            label_input=None,
            label_items=None,
            support_label_num_feature=None,
            test_label_num_feature=None,
            support_label_num_target=None,
            test_label_num_target=None
    ):
        self.gid = gid
        ...
        ''' sentence internal feature refer to label num '''
        self.support_label_num_feature = support_label_num_feature
        self.test_label_num_feature = test_label_num_feature
        self.support_label_num_target = support_label_num_target
        self.test_label_num_target = test_label_num_target
    ...
    def extract_label_num_feature(self, item: DataItem) -> torch.Tensor:
        ...
        if seq_in_text in self.seq_ins:
            ...
            if seq_in_text not in self.tag_data_dict:
                raise ValueError('the tag data dict is not complement')
...
class LabelNumInputBuilder(BertInputBuilder):

    def __init__(self, tokenizer, opt):
        super(LabelNumInputBuilder, self).__init__(tokenizer, opt)
...
class LabelNumFeatureConstructor(FeatureConstructor):
    def __init__(
            self,
            input_builder: InputBuilderBase,
            output_builder: OutputBuilderBase,
    ):
        super(LabelNumFeatureConstructor, self).__init__(input_builder, output_builder)
...
def make_preprocessor(opt):
    ...
    if opt.context_emb in transformer_style_embs:
        ...
        if opt.use_schema:
            if opt.label_num_schema:
                input_builder = LabelNumSchemaInputBuilder(tokenizer=tokenizer, opt=opt)
            else:
                input_builder = SchemaInputBuilder(tokenizer=tokenizer, opt=opt)
        else:
            if opt.label_num_schema:
                input_builder = LabelNumInputBuilder(tokenizer=tokenizer, opt=opt)
            else:
    ...
    if opt.use_schema:
        if opt.label_num_schema:
            preprocessor = LabelNumSchemaFeatureConstructor(input_builder=input_builder, output_builder=output_builder)
        else:
            preprocessor = SchemaFeatureConstructor(input_builder=input_builder, output_builder=output_builder)
    else:
        if opt.label_num_schema:
            preprocessor = LabelNumFeatureConstructor(input_builder=input_builder, output_builder=output_builder)
        else:
