...
BERT_BASE_UNCASED = '/your_bert_dir/bert-base-uncased/uncased_L-12_H-768_A-12/'
BERT_BASE_UNCASED_VOCAB = '/your_bert_dir/bert-base-uncased/uncased_L-12_H-768_A-12/vocab.txt'
...
def basic_args(parser):
    ...
    group.add_argument("--embedding_cache", type=str, default='/your_glove_dir/.word_vectors_cache',
    ...
    parser.add_argument("--task", default='mlc', choices=['mlc', 'sl', 'sc'],
                        help="Task: mlc:multi-label sent classify, sl:sequence labeling, sc:single label sent classify")
...
def model_args(parser):
    ...
    group.add_argument("--decoder", default='crf', type=str,
                       choices=['crf', 'sms', 'rule', 'sc', 'mlc', 'eamlc', 'msmlc', 'krnmsmlc'],
                       help="decode method")
    ...
    # Text classification decoder setting
    group.add_argument("--meta_rate", default=0.5, type=float, help="Trade-off: estimated vs. meta learnt threshold")
    group.add_argument("--threshold", default=0.6, type=float, help="Set default value of threshold")
    group.add_argument("--threshold_type", default='learn', choices=['fix', 'learn'],
                       help='decide threshold type.')
    group.add_argument("--use_location", default=False, action="store_true", help='decide to use location feature')
    group.add_argument("--proj_out_dim", default=10, type=int,
                       help='the projection dimension of label map projection text classifier')
    ...
    # Label num prediction setting
    group = parser.add_argument_group('SentenceFeature')
    group.add_argument("--label_num_schema", default=False, action="store_true",
                       help="Get the sentence internal features refer to label num")
    group.add_argument("--bandwidth", default=0.5, type=float, help="the kernel bandwidth in kernel regression network")
    group.add_argument("--kernel", default='gaussian', choices=['gaussian'], help="the kernel of kernel regression")
    group.add_argument("--kernel_learnable", default=False, action="store_true", help="whether to learn a bandwidth")
    group.add_argument("--tagger_model_dir", default='/your_tagger_dir/stanford-postagger-2018-10-16', type=str,
                       help="pos tagger model path")
    group.add_argument("--use_gold", default=False, action="store_true",
                       help="use the golden label num of test sentence target to guide model")
    group.add_argument("--ab_ea", default=False, action="store_true", help="ablation EA threshold")
    group.add_argument("--feature_map", default=False, action="store_true", help="whether map features")
    group.add_argument("--feature_num", default=5, type=int, help="the number of features")
    group.add_argument("--feature_map_dim", default=10, type=int, help="the dim of mapping features")
    group.add_argument("--feature_map_layer_num", default=1, type=int, help="the number of layers of feature map MLP")
    group.add_argument("--feature_map_act", default='none', type=str, choices=['none', 'relu', 'sigmoid', 'tanh'],
                       help="the activation of feature map")
