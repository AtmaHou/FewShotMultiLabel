...
class LabelNumFewShotTrainer(FewShotTrainer):
    def __init__(self, opt, optimizer, scheduler, param_to_optimize, device, n_gpu, tester=None):
        super(LabelNumFewShotTrainer, self).__init__(opt, optimizer, scheduler, param_to_optimize, device, n_gpu, tester)

    def get_data_loader(self, dataset, sampler):
        pad_collate = PadCollate(dim=-1, sp_dim=-2, sp_item_idx=[3, 8, 12, 14])  # nwp_index, spt_tgt need special padding
        data_loader = DataLoader(dataset, sampler=sampler, batch_size=self.batch_size, collate_fn=pad_collate)
        return data_loader
    ...
    def unpack_feature(self, feature: FewShotFeature) -> List[torch.Tensor]:
        ret = [
            ...
            torch.LongTensor([len(feature.support_feature_items)]),  # support num
            # sentence feature
            feature.support_label_num_feature,  # 14
            feature.test_label_num_feature,  # 15
            feature.support_label_num_target,  # 16
            feature.test_label_num_target  # 17
        ]
        return ret
